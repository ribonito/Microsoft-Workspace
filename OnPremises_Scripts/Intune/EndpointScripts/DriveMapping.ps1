<#
.SYNOPSIS
    INT-018 | Intune Endpoint Script - Map Network Drives at User Logon (JSON-driven, Group-filtered).

.DESCRIPTION
    Maps network drives for Intune-managed devices at each user logon using a JSON
    configuration file. Supports AD group-based filtering so different drives can be
    mapped for different user groups.

    When running under SYSTEM (Intune IME), creates a scheduled task that runs at each
    user logon. When running as user, reads the JSON mapping config and applies drives.

    JSON config path (user context):
        %APPDATA%\Sunrise\DriveMappings.json

    Features:
        - AD group-filtered drive assignments
        - Stale drive removal (if RemoveStaleDrives = true in JSON)
        - Persistent mapped drives (survive logoff)
        - Silent window (VBScript wrapper used for scheduled task)
        - Triggers: logon + network profile change events (10002, 4004)

.PRODUCT
    Microsoft Intune / Windows Endpoint / Network Drive Mapping

.ORIGINAL_AUTHOR
    Nicola Suter, nicolonsky tech: https://tech.nicolonsky.ch
    Auto-generated by: https://intunedrivemapping.azurewebsites.net

.MAINTAINER
    Josep Canas - M365 Solutions Architect (INT-018 classification & English header)
...
.NOTES
    ...
    - Logs to: %ProgramData%\Microsoft\IntuneManagementExtension\Logs\IntuneDriveMappingScheduledTask.log
...
if (-not (Test-RunningAsSystem)) {
    ...
    $MappingJson = "$env:APPDATA\Sunrise\DriveMappings.json"
    if (-not (Test-Path $MappingJson)) {
        Write-Warning "File '$MappingJson' doesn't exist. Drive mapping will not be done."
        Exit 0
    }

    $driveMappings = Get-Content -LiteralPath $MappingJson -Raw | ConvertFrom-Json -ErrorAction Stop

    #used to create an array for groups
    $driveMappingConfig = foreach ($d in $($driveMappings.Mappings)) {
        [PSCustomObject]@{
            Path        = $($d.Path)
    ...
    Write-Output "removeStaleDrives: $($driveMappings.RemoveStaleDrives)"
    if($driveMappings.RemoveStaleDrives -eq "true"){
...
if (Test-RunningAsSystem) {
	...
	Start-Transcript -Path $(Join-Path -Path "$env:ProgramData\Microsoft\IntuneManagementExtension\Logs" -ChildPath "IntuneDriveMappingScheduledTask.log")
	...
	$schtaskName = "SRMW-IntuneDriveMapping"
