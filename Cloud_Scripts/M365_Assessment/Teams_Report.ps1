...
{        
        ...
        $TeamNickname = (Get-Team | ?{$_.GroupID -eq $Team}).Mailnickname 
...
$ChannelCount = (Get-TeamChannel -GroupId $Team).ID.Count 
        ...
        $TeamList = $TeamList + [PSCustomObject]@{TeamName = $TeamName; TeamNickname = $TeamNickname; TeamObjectID = $TeamGUID; TeamOwners = $TeamOwner -join ', '; TeamMemberCount = $TeamUserCount; NoOfChannels = $ChannelCount; ChannelNames = $TeamChannels -join ', '; SharePointSite = $TeamGroup.SharePointSiteURL; AccessType = $TeamGroup.AccessType; TeamGuests = $TeamGuest -join ','; TeamMembers = $TeamMembers -join ','}
