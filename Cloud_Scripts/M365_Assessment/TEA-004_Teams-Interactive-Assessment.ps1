<#
.SYNOPSIS
    TEA-004 | Microsoft Teams - Interactive Teams and Channels Assessment Tool.

.DESCRIPTION
    Provides an interactive command-line menu to query and export Microsoft Teams
    and channels inventory, membership, ownership, and visibility details.
    Outputs reports directly to timestamped CSV files.

    Menu Options:
        1. All Teams in organization
        2. All Teams members and owners report
        3. Specific Team's members and Owners report
        4. All Teams' owners report
        5. Specific Team's owners report
        6. All channels in organization
        7. All channels in specific Team
        8. Members and Owners Report of Single Channel
        0. Exit

    Authentication options:
        - MFA-enabled credentials flow
        - Stored credentials or parameters (-UserName and -Password)
        - Interactive credentials prompt (default)

.PRODUCT
    Microsoft Teams

.ORIGINAL_AUTHOR
    O365Reports Team (o365reports.com)
    Reference: https://o365reports.com/2020/05/28/microsoft-teams-reporting-using-powershell/

.MAINTAINER
    Josep Canas - M365 Solutions Architect (TEA-004 classification & English header)

.VERSION
    2.0

.PARAMETER UserName
    UPN of the administrator account for non-MFA authentication.

.PARAMETER Password
    Cleartext password for non-MFA authentication (converted internally).

.PARAMETER MFA
    Switch. Force interactive prompt suitable for MFA-enabled accounts.

.PARAMETER Action
    Directly execute one of the menu numbers (1-8) and exit instead of showing the interactive menu.

.EXAMPLE
    # Run the interactive menu
    .\TEA-004_Teams-Interactive-Assessment.ps1

.EXAMPLE
    # Execute report 1 directly and exit
    .\TEA-004_Teams-Interactive-Assessment.ps1 -Action 1
#>
