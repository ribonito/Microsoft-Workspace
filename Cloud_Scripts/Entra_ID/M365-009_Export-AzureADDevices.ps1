<#
.SYNOPSIS
    M365-009 | Entra ID / Microsoft Graph - Export Azure AD Devices Report.

.DESCRIPTION
    Retrieves all devices registered in Azure Active Directory (Entra ID) using
    the Microsoft Graph Beta PowerShell module and exports them to a CSV file.
    Supports filtering by active/inactive, managed/unmanaged, enabled/disabled,
    and checks if BitLocker recovery keys exist for the devices.

    Authentication options:
        - Interactive or MFA authentication (delegated)
        - Certificate-based authentication (app-only, requires TenantId, ClientId, CertificateThumbprint)

.PRODUCT
    Microsoft Entra ID / Microsoft Graph / Devices

.ORIGINAL_AUTHOR
    AdminDroid Community (o365reports.com)
    Reference: https://o365reports.com/2023/04/18/get-azure-ad-devices-report-using-powershell/

.MAINTAINER
    Josep Canas - M365 Solutions Architect (M365-009 classification & English header)

.VERSION
    2.0

.PARAMETER TenantId
    The Azure Tenant ID (GUID) for Certificate-based Authentication.

.PARAMETER ClientId
    The App Registration Client ID (Application ID) for Certificate-based Authentication.

.PARAMETER CertificateThumbprint
    The Thumbprint of the certificate installed on the machine for authentication.

.PARAMETER EnabledDevice
    Switch. If present, filters report to only show enabled devices.

.PARAMETER DisabledDevice
    Switch. If present, filters report to only show disabled devices.

.PARAMETER InactiveDays
    Integer. Filters the report to show devices inactive for more than N days.

.PARAMETER ManagedDevice
    Switch. If present, filters report to only show managed devices.

.PARAMETER DevicesWithBitLockerKey
    Switch. If present, filters report to only show devices with BitLocker keys.

.EXAMPLE
    # Interactive run with Graph authentication
    .\M365-009_Export-AzureADDevices.ps1

.EXAMPLE
    # Certificate-based Authentication run
    .\M365-009_Export-AzureADDevices.ps1 -TenantId "xxx" -ClientId "yyy" -CertificateThumbprint "zzz"
#>
