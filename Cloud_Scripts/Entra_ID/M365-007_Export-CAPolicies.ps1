<#
.SYNOPSIS
    M365-007 | M365 Assessment - Export Conditional Access Policies to CSV.

.DESCRIPTION
    Retrieves all Entra ID Conditional Access policies via Microsoft Graph Beta
    and exports them to a detailed CSV report containing 33 attributes.
...
.AUTHOR
    ...
    Josep Canas - M365 Solutions Architect (M365-007 classification & template)

.VERSION
    2.1

.PARAMETER ActiveCAPoliciesOnly
    Filter to export only active (Enabled) CA policies.

.PARAMETER DisabledCAPoliciesOnly
    Filter to export only disabled CA policies.

.PARAMETER ReportOnlyMode
    Filter to export only policies configured in report-only mode.

.PARAMETER RecentlyCreatedCAPolicies
    Filter to select policies created within the specified number of days.

.PARAMETER RecentlyModifiedCAPolicies
    Filter to select policies modified within the specified number of days.

.PARAMETER CreateSession
    If set, disconnects existing Graph sessions before starting.

.PARAMETER TenantId
    The Tenant ID to authenticate against (for certificate authentication).

.PARAMETER ClientId
    The Client ID (App ID) to authenticate with (for certificate authentication).

.PARAMETER CertificateThumbprint
    The certificate thumbprint to authenticate with (for certificate authentication).

.EXAMPLE
    .\M365-007_Export-CAPolicies.ps1

.NOTES
    - Requires Microsoft.Graph.Beta module.
    - Reference: https://o365reports.com/2024/02/20/export-conditional-access-policies-to-excel-using-powershell
#>
...
#region ── Execution & Report Export ──────────────────────────────────────────
