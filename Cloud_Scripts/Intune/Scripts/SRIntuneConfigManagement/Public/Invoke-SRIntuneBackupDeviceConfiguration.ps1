function Invoke-SRIntuneBackupDeviceConfiguration {
    <#
    .SYNOPSIS
    ...
    .EXAMPLE
    Invoke-SRIntuneBackupDeviceConfiguration -Path "C:\temp"
    #>
        ...
        if (($deviceConfiguration.'@odata.type' -eq '#microsoft.graph.windows10CustomConfiguration') -and ($deviceConfiguration.omaSettings | Where-Object { $_.isEncrypted -contains $true } )) {
            ...
            foreach ($omaSetting in $deviceConfiguration.omaSettings) {
                ...
                if ($omaSetting.isEncrypted) {
                    $Uri = "$ApiVersion/deviceManagement/deviceConfigurations/$($deviceConfiguration.id)/getOmaSettingPlainTextValue(secretReferenceValueId='$($omaSetting.secretReferenceValueId)')"
                    $omaSettingValue = Invoke-MgGraphRequest -Uri $Uri | Get-MgGraphAllPages
                }
...
#Invoke-SRIntuneBackupDeviceConfiguration -Path "C:\temp\IntuneBackup\FunctionTest"