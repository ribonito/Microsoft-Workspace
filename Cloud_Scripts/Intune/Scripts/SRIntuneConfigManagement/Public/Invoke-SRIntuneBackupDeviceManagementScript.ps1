function Invoke-SRIntuneBackupDeviceManagementScript {
    <#
    .SYNOPSIS
    ...
    .EXAMPLE
    Invoke-SRIntuneBackupDeviceManagementScript -Path "C:\temp"
    #>
    ...
    foreach ($deviceManagementScript in $deviceManagementScripts) {
        ...
        $Uri = "$ApiVersion/deviceManagement/deviceManagementScripts/$($deviceManagementScript.Id)"
        $deviceManagementScriptObject = Invoke-MgGraphRequest -Uri $Uri
...
#Invoke-SRIntuneBackupDeviceManagementScript -Path "C:\temp\IntuneBackup\FunctionTest"