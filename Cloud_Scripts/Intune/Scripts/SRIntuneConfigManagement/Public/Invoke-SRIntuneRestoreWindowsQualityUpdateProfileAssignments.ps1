function Invoke-SRIntuneRestoreWindowsQualityUpdateProfileAssignments {
    ...
    if (-not (Test-Path "$Path\Windows Quality Update Profiles\Assignments")) {
        Write-Warning "Folder '$Path\Windows Quality Update Profiles\Assignments' doesn't exist. Skipping restore of Windows Quality Update Profile Assignments"
        Return
    }
